...
### 功能三：手動輸入 CPE 並查詢漏洞
1. 允許使用者手動輸入完整的 CPE 2.3 格式編號
2. 驗證 CPE 格式的正確性
3. 解析 CPE 編號並顯示詳細資訊
4. 自動查詢 NVD (National Vulnerability Database) 中與該 CPE 相關的 CVE 漏洞
5. 顯示漏洞數量、CVE ID、嚴重性等級和描述
6. 支援匯出包含漏洞資訊的資料
...
3. **手動輸入 CPE 並查詢漏洞**
   - 在「功能三」區塊輸入完整的 CPE 2.3 格式編號
   - 例如：`cpe:2.3:a:microsoft:windows:10:*:*:*:*:*:*:*`
   - 點擊「驗證並查詢漏洞」按鈕
   - 系統會驗證 CPE 格式並解析資訊
   - 自動查詢 NVD 資料庫中相關的 CVE 漏洞
   - 顯示漏洞數量、CVE ID、嚴重性等級等資訊
   - 如果發現漏洞，會以紅色標示；無漏洞則顯示綠色勾號
