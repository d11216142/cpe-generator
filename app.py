...
# ---------------------------------------------------------------------------
# Feature 3: CPE vs. Vulnerability Database Comparison
# ---------------------------------------------------------------------------

# Sample CVE entries used as fallback when the NVD API is unavailable.
# Keyed by vendor name (lower-case).
SAMPLE_CVE_DATA = {
    'microsoft': [
        {'cve_id': 'CVE-2024-21412', 'description': 'Windows Internet Shortcut Files Security Feature Bypass Vulnerability', 'severity': 'HIGH', 'cvss_score': 8.1, 'published_date': '2024-02-13'},
        {'cve_id': 'CVE-2024-21351', 'description': 'Windows SmartScreen Security Feature Bypass Vulnerability', 'severity': 'MEDIUM', 'cvss_score': 7.6, 'published_date': '2024-02-13'},
    ],
    'google': [
        {'cve_id': 'CVE-2024-0519', 'description': 'Out of bounds memory access in V8 in Google Chrome prior to 120.0.6099.224', 'severity': 'HIGH', 'cvss_score': 8.8, 'published_date': '2024-01-16'},
        {'cve_id': 'CVE-2024-0518', 'description': 'Type Confusion in V8 in Google Chrome prior to 120.0.6099.224', 'severity': 'HIGH', 'cvss_score': 8.8, 'published_date': '2024-01-16'},
    ],
    'mozilla': [
        {'cve_id': 'CVE-2024-0741', 'description': 'Out of bounds write in ANGLE in Mozilla Firefox before 122.0', 'severity': 'HIGH', 'cvss_score': 7.5, 'published_date': '2024-01-23'},
        {'cve_id': 'CVE-2024-0753', 'description': 'HSTS policy violation via fetch redirect in Mozilla Firefox before 122.0', 'severity': 'MEDIUM', 'cvss_score': 6.5, 'published_date': '2024-01-23'},
    ],
    'adobe': [
        {'cve_id': 'CVE-2024-20745', 'description': 'Adobe Premiere Pro Memory Corruption Remote Code Execution Vulnerability', 'severity': 'CRITICAL', 'cvss_score': 9.8, 'published_date': '2024-03-12'},
        {'cve_id': 'CVE-2023-44338', 'description': 'Adobe Acrobat Reader Out-Of-Bounds Read Vulnerability', 'severity': 'HIGH', 'cvss_score': 7.8, 'published_date': '2023-11-14'},
    ],
    'apache': [
        {'cve_id': 'CVE-2023-25690', 'description': 'HTTP request splitting with mod_rewrite and mod_proxy in Apache HTTP Server', 'severity': 'CRITICAL', 'cvss_score': 9.8, 'published_date': '2023-03-07'},
        {'cve_id': 'CVE-2023-44487', 'description': 'HTTP/2 rapid reset attack in multiple servers including Apache HTTP Server', 'severity': 'HIGH', 'cvss_score': 7.5, 'published_date': '2023-10-10'},
    ],
    'oracle': [
        {'cve_id': 'CVE-2024-20926', 'description': 'Vulnerability in the Oracle Java SE product', 'severity': 'MEDIUM', 'cvss_score': 5.9, 'published_date': '2024-01-16'},
        {'cve_id': 'CVE-2023-22067', 'description': 'Vulnerability in Oracle Java SE, Oracle GraalVM for JDK', 'severity': 'MEDIUM', 'cvss_score': 5.3, 'published_date': '2023-10-17'},
    ],
    'cisco': [
        {'cve_id': 'CVE-2024-20272', 'description': 'Cisco Unity Connection Unauthenticated Arbitrary File Upload Vulnerability', 'severity': 'CRITICAL', 'cvss_score': 9.8, 'published_date': '2024-01-24'},
        {'cve_id': 'CVE-2023-20269', 'description': 'Cisco Adaptive Security Appliance and Firepower Threat Defense Zero-Day', 'severity': 'HIGH', 'cvss_score': 8.8, 'published_date': '2023-09-06'},
    ],
    'vmware': [
        {'cve_id': 'CVE-2024-22245', 'description': 'VMware Enhanced Authentication Plugin Authentication Relay Attack', 'severity': 'CRITICAL', 'cvss_score': 9.6, 'published_date': '2024-02-20'},
        {'cve_id': 'CVE-2023-34048', 'description': 'VMware vCenter Server Out-of-Bounds Write Vulnerability', 'severity': 'CRITICAL', 'cvss_score': 9.8, 'published_date': '2023-10-25'},
    ],
    'python': [
        {'cve_id': 'CVE-2023-6597', 'description': 'Path traversal vulnerability in tempfile.TemporaryDirectory in CPython', 'severity': 'HIGH', 'cvss_score': 7.8, 'published_date': '2024-03-19'},
        {'cve_id': 'CVE-2023-40217', 'description': 'TLS handshake bypass via crafted invalid handshake packet in Python ssl module', 'severity': 'MEDIUM', 'cvss_score': 5.3, 'published_date': '2023-08-25'},
    ],
    'nodejs': [
        {'cve_id': 'CVE-2024-21892', 'description': 'Code injection and privilege escalation via Linux capabilities in Node.js', 'severity': 'HIGH', 'cvss_score': 7.8, 'published_date': '2024-02-14'},
        {'cve_id': 'CVE-2024-22019', 'description': 'HTTP/2 continuation flood in Node.js', 'severity': 'HIGH', 'cvss_score': 7.5, 'published_date': '2024-04-03'},
    ],
}


def query_nvd_for_cve(cpe_string):
    """
    向 NVD API 查詢指定 CPE 的相關漏洞 (CVE)。
    若 API 不可用，回退至內建的示範漏洞資料。

    Args:
        cpe_string: CPE 字串

    Returns:
        list: 漏洞資料列表，每筆包含 cve_id, description, severity,
    ...
    try:
                ...
                for metric_key in ('cvssMetricV31', 'cvssMetricV30', 'cvssMetricV2'):
                    ...
                    if metric_list:
                        ...
                        severity = (
                            ...
                            or metric_list[0].get('baseSeverity', 'N/A')
                        )
                        break
        ...
        print(f"NVD API 查詢失敗，改用示範資料: {e}")

    # Fallback: use built-in sample data matched by vendor name
    parts = cpe_string.split(':')
    vendor = parts[3].lower() if len(parts) > 3 else ''
    return SAMPLE_CVE_DATA.get(vendor, [])


@app.route('/api/compare-vulnerabilities', methods=['POST'])
def compare_vulnerabilities():
    """
    從資料庫取出 CPE 資料，與 NVD 漏洞資料庫進行比對，
    並回傳比對結果。
    """
    try:
        records, error_msg = get_cpe_from_database()
        if error_msg:
            return jsonify({'error': error_msg}), 500

        if not records:
            return jsonify({
                'error': '資料庫中沒有 CPE 資料，請先在功能一中儲存資料到資料庫'
            }), 404

        comparison_results = []
        for idx, record in enumerate(records):
            cpe_string = record.get('cpe', '')
            if not cpe_string:
                continue

            # Throttle requests to respect NVD rate limit (5 req / 30 s = ~6 s per request)
            if idx > 0:
                time.sleep(6)

            vulnerabilities = query_nvd_for_cve(cpe_string)
            comparison_results.append({
                'cpe': cpe_string,
                'vendor': record.get('vendor', ''),
                'product': record.get('product', record.get('product_name', '')),
        ...
        return jsonify({
            'total_cpe': len(comparison_results),
            'total_cve': sum(r['cve_count'] for r in comparison_results),
...
@app.route('/api/export-report-csv', methods=['POST'])
def export_report_csv():
    ...
    try:
        ...
        if not results:
            return jsonify({'error': '沒有比對結果可以匯出'}), 400

        output = io.StringIO()
        writer = csv.writer(output)
        writer.writerow([
            'CPE', '供應商', '產品名稱', '版本號',
            'CVE ID', '嚴重等級', 'CVSS 分數', '描述', '發布日期',
        ])
        ...
        return send_file(
            ...
            mimetype='text/csv',
            as_attachment=True,
            download_name=f'vulnerability_report_{datetime.now().strftime("%Y%m%d_%H%M%S")}.csv',
        )
    except Exception as e:
        return jsonify({'error': str(e)}), 500


@app.route('/api/export-report-xlsx', methods=['POST'])
    ...
    try:
        ...
        for item in results:
            if item.get('vulnerabilities'):
                for vuln in item['vulnerabilities']:
                    ws.cell(row=row_num, column=1, value=item.get('cpe', ''))
                    ws.cell(row=row_num, column=2, value=item.get('vendor', ''))
                    ws.cell(row=row_num, column=3, value=item.get('product', ''))
                    ws.cell(row=row_num, column=4, value=item.get('version', ''))
                    ws.cell(row=row_num, column=5, value=vuln.get('cve_id', ''))
                    severity = vuln.get('severity', 'N/A')
                    sev_cell = ws.cell(row=row_num, column=6, value=severity)
                    color = severity_colors.get(severity.upper(), 'AAAAAA')
                    sev_cell.fill = PatternFill(
                        start_color=color, end_color=color, fill_type="solid"
                    )
                    ws.cell(row=row_num, column=7, value=vuln.get('cvss_score', ''))
                    ws.cell(row=row_num, column=8, value=vuln.get('description', ''))
                    ws.cell(row=row_num, column=9, value=vuln.get('published_date', ''))
                    row_num += 1
            else:
                ws.cell(row=row_num, column=1, value=item.get('cpe', ''))
                ws.cell(row=row_num, column=2, value=item.get('vendor', ''))
                ws.cell(row=row_num, column=3, value=item.get('product', ''))
                ws.cell(row=row_num, column=4, value=item.get('version', ''))
                ws.cell(row=row_num, column=5, value='無漏洞')
                row_num += 1
        ...
        return send_file(
            ...
            mimetype='application/vnd.openxmlformats-officedocument.spreadsheetml.sheet',
            as_attachment=True,
            download_name=f'vulnerability_report_{datetime.now().strftime("%Y%m%d_%H%M%S")}.xlsx',
        )
    except Exception as e:
        return jsonify({'error': str(e)}), 500


@app.route('/api/export-report-json', methods=['POST'])
def export_report_json():
    ...
    try:
        ...
        json_output = {
            'export_date': datetime.now().strftime('%Y-%m-%d %H:%M:%S'),
            'total_cpe': len(results),
            'total_cve': sum(r.get('cve_count', 0) for r in results),
            'results': results,
        }
        json_string = json.dumps(json_output, ensure_ascii=False, indent=2)
        ...
        return send_file(
            ...
            mimetype='application/json',
            as_attachment=True,
            download_name=f'vulnerability_report_{datetime.now().strftime("%Y%m%d_%H%M%S")}.json',
        )
    except Exception as e:
        return jsonify({'error': str(e)}), 500
